...
| Scripted/local/isolated execution | Experimental executor contract and no-Atlas vertical slice implemented; local host execution is explicitly not a security sandbox. A live-attested Docker Desktop backend can run exact digest-pinned cached Linux images with no network, host mounts, or daemon socket and strict non-root/resource controls; it remains an OCI container sharing the Docker VM kernel, not a per-workload microVM |
...
| `sova detonate` | Real Playwright/Chrome website execution plus bounded trusted local-process execution on two credential-stripped copies; both require exact human approval and produce signed primary/reproduction traces plus evidence capsules. The isolated executor API now supports bounded untrusted Linux command execution without host mounts; native desktop UI detonation and per-workload microVM admission remain unsupported |
...
| Remaining validation | Authorized non-fixture targets, native desktop UI isolation, a passing per-workload microVM backend, paid/credentialed real-provider Arena runs, independent readers/adapters/reviewers, cross-provider comparisons, larger nondeterministic studies, and promoted release decisions |
