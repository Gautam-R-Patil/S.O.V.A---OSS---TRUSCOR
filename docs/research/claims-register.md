...
| P-07 | The implemented 77-leaf CLI exposes `map`, `check`, `compose`, `arena`, `rehearse`, `forensics`, `evidence`, `case build`, `sentinel`, `registry verify`, `adjudicate`, `disclose`, authorization-gated browser detonation, bounded real-browser trigger hunting, provider-assisted tool-isolated browser campaign planning, and controlled browser counterfactual studies; general consequential MCP operations remain separately exact-gated. | VERIFIED | Release owner | parser and command tests; local-MCP tests; deterministic provider-role integration; installed-Chrome detonation, campaign, dynamic-check, and counterfactual lanes | 2026-08-07 | Every release | Provider-role integration is verified with deterministic transports; no real external provider run is claimed. Availability is bounded to documented inputs and backends. |
