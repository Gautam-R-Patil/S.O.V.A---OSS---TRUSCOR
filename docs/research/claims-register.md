...
| P-07 | The implemented 73-leaf CLI exposes `map`, `check`, `compose`, `arena`, `rehearse`, `forensics`, `evidence`, `sentinel`, `registry verify`, `adjudicate`, `disclose`, authorization-gated browser detonation, and bounded real-browser trigger hunting; general offensive MCP operations remain separately exact-gated. | VERIFIED | Release owner | parser and command tests; local-MCP tests; installed-Chrome detonation and campaign acceptance lanes | 2026-08-07 | Every release | Availability is bounded to documented inputs and backends; it is not evidence that every optional live executor/provider/target was exercised. |
