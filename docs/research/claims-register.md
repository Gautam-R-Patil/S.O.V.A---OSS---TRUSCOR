...
| P-07 | The implemented 110-leaf CLI exposes `map`, `check`, `compose`, `arena`, `rehearse`, `forensics`, evidence-native static/live replay, blinded causal-study preparation/scoring, `evidence`, `case build`, `sentinel`, durable `monitor`, offline and self-hosted `registry`, `extension`, `adjudicate`, `disclose`, Docker isolation attestation, opaque browser-session provisioning/handoff, authorization-gated browser and local-software detonation, bounded real-browser trigger hunting, provider-assisted tool-isolated browser planning and rehearsal, a real-time Arena chamber, authorized website Arena, executor-backed browser swarm, and controlled browser counterfactual studies; general consequential MCP operations remain separately exact-gated. | VERIFIED | Release owner | parser and command tests; local-MCP tests; deterministic provider-role integration; replay/causal/community/monitor service tests; real local-process, Docker Desktop OCI, and installed-Chrome detonation, campaign, persistent-session, browser-swarm, dynamic-check, Arena, counterfactual, and replay-application lanes | 2026-08-19 | Every release | Provider-role integration is verified with deterministic transports; no paid real external-provider run is claimed. Availability is bounded to documented inputs and backends. |
