...
| P-07 | The implemented 93-leaf CLI exposes `map`, `check`, `compose`, `arena`, `rehearse`, `forensics`, `evidence`, `case build`, `sentinel`, `registry verify`, `extension`, `adjudicate`, `disclose`, Docker isolation attestation, opaque browser-session provisioning/handoff, authorization-gated browser and local-software detonation, bounded real-browser trigger hunting, provider-assisted tool-isolated browser planning and rehearsal, a real-time Arena chamber and authorized website Arena, and controlled browser counterfactual studies; general consequential MCP operations remain separately exact-gated. | VERIFIED | Release owner | parser and command tests; local-MCP tests; deterministic provider-role integration; real local-process, Docker Desktop OCI, and installed-Chrome detonation, campaign, persistent-session, dynamic-check, Arena, and counterfactual lanes | 2026-08-09 | Every release | Provider-role integration is verified with deterministic transports; no paid real external-provider run is claimed. Availability is bounded to documented inputs and backends. |
