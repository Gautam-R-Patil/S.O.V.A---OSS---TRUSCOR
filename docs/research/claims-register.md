...
| P-07 | The implemented 111-leaf CLI exposes `map`, `check`, `compose`, `arena`, `rehearse`, `forensics`, evidence-native static/live replay, blinded causal-study preparation/scoring, `evidence`, `case build`, `sentinel`, durable `monitor`, offline and self-hosted `registry`, `extension`, `adjudicate`, `disclose`, Docker isolation attestation, opaque browser-session provisioning/handoff, authorization-gated browser and local-software detonation, bounded real-browser trigger hunting, provider-assisted tool-isolated browser planning and rehearsal, a real-time Arena chamber, authorized website Arena, executor-backed browser swarm, and controlled browser counterfactual studies; general consequential MCP operations remain separately exact-gated. | VERIFIED | Release owner | parser and command tests; local-MCP tests; deterministic provider-role integration; replay/causal/community/monitor service tests; real local-process, Docker Desktop OCI, and installed-Chrome detonation, campaign, persistent-session, browser-swarm, dynamic-check, Arena, counterfactual, and replay-application lanes; [bounded AI Goat provider acceptance](../engineering/aigoat-loopback-acceptance-2026-08-20.md) | 2026-08-20 | Every release | Provider-role integration has deterministic mandatory tests and one founder-authorized OpenRouter free-model loopback AI Goat Challenge 1 acceptance. This is not paid-provider quality evidence, general target compatibility, or autonomous whole-platform coverage. Availability is bounded to documented inputs and backends. |
...
| P-49 | SOVA's inert replay application can navigate and compare integrity-checked observable trace events through play/pause, speed, scrubbing, sensor-family lanes, search, and recorded links; verify a media-digest-bound passing-oracle cue, select its exact trace event, and open a bounded decisive video window; and tail an integrity-valid unsealed writer prefix through a bounded capability-URL loopback SSE service that visibly transitions only after the finalized trace verifies. | VERIFIED | Replay owner | SRC-PERFETTO; SRC-SSE; SRC-PYHTTP; hostile-content, tamper, cue-substitution, live-prefix, SSE, CLI, deterministic browser-campaign, and real-Chrome interaction tests; [replay-application specification](../specifications/evidence-replay-application-0.1.md) | 2026-08-19 | Every renderer, trace, browser, recording, cue, or service change | It executes no action and proves neither sensor/recorder truth nor complete observability. Cue timing is bounded by a same-host recorder-start RPC but frames are not independently cryptographically timestamped. The reference HTTP service is not Internet-production infrastructure, and synchronized time proximity is not causal inference. |
