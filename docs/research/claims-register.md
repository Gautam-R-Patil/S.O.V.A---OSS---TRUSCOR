...
| P-07 | The implemented 74-leaf CLI exposes `map`, `check`, `compose`, `arena`, `rehearse`, `forensics`, `evidence`, `sentinel`, `registry verify`, `adjudicate`, `disclose`, authorization-gated browser detonation, bounded real-browser trigger hunting, and provider-assisted tool-isolated browser campaign planning; general consequential MCP operations remain separately exact-gated. | VERIFIED | Release owner | parser and command tests; local-MCP tests; deterministic provider-role integration; installed-Chrome detonation and campaign acceptance lanes | 2026-08-07 | Every release | Provider-role integration is verified with deterministic transports; no real external provider run is claimed. Availability is bounded to documented inputs and backends. |
