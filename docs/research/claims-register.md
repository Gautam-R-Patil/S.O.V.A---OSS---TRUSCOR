...
| P-07 | The implemented 71-leaf CLI exposes `map`, `check`, `compose`, `arena`, `rehearse`, `forensics`, `evidence`, `sentinel`, `registry verify`, `adjudicate`, `disclose`, and authorization-gated browser detonation; general offensive MCP operations remain separately exact-gated. | VERIFIED | Release owner | parser and command tests; local-MCP tests; real-browser acceptance lane | 2026-08-07 | Every release | Availability is bounded to documented inputs and backends; it is not evidence that every optional live executor/provider/target was exercised. |
