...
| P-07 | The implemented CLI exposes `map`, `check`, `compose`, `arena`, `rehearse`, `forensics`, `evidence`, `sentinel`, `registry verify`, `adjudicate`, and `disclose`; exact-gated `detonate` is a local MCP tool. | VERIFIED | Release owner | parser-discovered 67-command coverage audit; command and local-MCP tests | 2026-08-04 | Every release | Availability is bounded to documented inputs and backends; it is not evidence that every optional live executor/provider/target was exercised. |
